...
def real_cli_fixture(root: Path, monkeypatch: pytest.MonkeyPatch, mode: str) -> None:
    ...
    codex.write_text(
        ...
        "'verdict':'pass' if role=='review' else ''}\n"
        "pathlib.Path(a[a.index('--output-last-message')+1]).write_text(json.dumps(data))\n"
    )
    codex.chmod(0o755)
    venv_bin = root / ".venv/bin"
    venv_bin.mkdir(parents=True)
    checker = venv_bin / "python"
    checker.write_text(
        "#!/usr/bin/env python3\n"
        ...
        "if mode=='cancelcheck':\n"
        " pathlib.Path('.run/harness/demo/check-started').write_text('yes')\n"
        " time.sleep(30)\n"
    )
    checker.chmod(0o755)
...
def test_real_cli_sigint_during_check_stops_attempts(
    ...
    try:
        ...
        while (
            not (root / ".run/harness/demo/check-started").exists() and time.monotonic() < deadline
        ):
            time.sleep(0.05)
        assert (root / ".run/harness/demo/check-started").exists()
